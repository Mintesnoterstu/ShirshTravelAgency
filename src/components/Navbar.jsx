        ...
        <div style={{ display: 'flex', gap: '2rem', alignItems: 'center' }}>
          {navLinks.map(link => (
            <Link
              ...
              to={link.to}
              style={{
                color: location.pathname === link.to ? currentTheme.accent : currentTheme.navbarText,
                textDecoration: 'none',
                fontWeight: location.pathname === link.to ? 'bold' : '500',
                fontSize: '1.05rem',
                padding: '0.5rem 1rem',
                borderRadius: '8px',
                background: location.pathname === link.to ? currentTheme.navbarActive : 'transparent',
                boxShadow: location.pathname === link.to ? `0 2px 8px ${currentTheme.shadow}` : 'none',
                transition: 'all 0.2s',
                position: 'relative',
                outline: 'none',
                cursor: 'pointer'
